...
// Next may use port 3001 when port 3000 is already occupied. Keep both local
// origins trusted without weakening the production origin configuration.
...
export const auth = betterAuth({
  ...
  // Google verifies the email address returned during OAuth. Trusting it here
  // lets an existing email/password account sign in with Google as well.
  account: {
    accountLinking: {
      enabled: true,
      trustedProviders: ["google"],
      // Email/password verification is disabled for this portal; Google still
      // supplies a verified email during the trusted OAuth flow.
